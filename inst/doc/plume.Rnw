...
A rule of thumb is that the wind must be blowing at least 1.0 meters per second (m/s) for our model to be sensible. Under these conditions, the transport of the plume is dominated by advection, and diffusion along the wind vector is negligible. 

<<define_plume>>=
plume <- GaussianPlume(Q=50, H=10, u=2, sigma=PasquillGifford('D'))
@

Diffusion does occur, however. The diffusivity field $\sigma$ describes the lateral ($\sigma_y$) and vertical ($\sigma_z$) diffusivity at distance $x$ along the wind vector. In theory, any \verb@sigma@ function may be supplied as an argument to \verb@GaussianPlume@, as long as it accepts a numeric vector $x$ and returns a list with numeric $y$ and $z$ components. 
...
ggplot(subset(cells, mgm3 > 0.1), aes(x, y)) + 
	...
	scale_x_continuous(limits=c(-max(x), max(x)), expand=c(0, 0)) +
